...
class ContactUs extends React.Component {
  constructor(props) {
    ...
    this.state = { showMessageSent: false };
  }
  ...
  handleSubmit = async (event) => {
    ...
    if (response.ok) {
      ...
      this.setState({ showMessageSent: true });
  ...
  render() {
    return (
      <div className="w-full mt-12 md:mt-0 lg:h-[1006px] lg:px-16 px-8 lg:py-28 py-0 flex-col justify-center items-center lg:gap-20 inline-flex">
        ...
        <form onSubmit={this.handleSubmit} className="lg:h-[553px] flex-col lg:px-36 w-full justify-start items-center lg:gap-6 flex">
          <label className="flex flex-col items-start self-stretch justify-start h-20 gap-2">
            ...
            <input type="text" name="name" className="self-stretch w-full p-3 bg-white border border-white" />
          </label>
          <label className="flex flex-col items-start self-stretch justify-start h-20 gap-2">
            <span className="self-stretch text-white text-base font-normal font-['Poppins'] leading-normal">Email</span>
            <input ref={this.emailRef} type="email" name="email" className="self-stretch p-3 bg-white border border-white" />
          </label>
          <label className="self-stretch h-[212px] flex-col justify-start items-start gap-2 flex">
            ...
            <textarea ref={this.queryRef} name="message" className="self-stretch h-[180px] p-3 bg-white border border-white justify-start items-start inline-flex"></textarea>
          </label>
...
{this.state.showMessageSent && <div className="alert-green">Your message has been sent!</div>}
        </form>
      </div>
    );
  }
